...
class TSTSentimentAnalyzer:
    ...
    MODIFIED: Filters to only process specified fractal dimension post IDs
    """
    ...
    def __init__(self, max_level=4):
        ...
        # Define the specific post IDs to analyze
        self.HIGHEST_FRACTAL_POST_IDS = ['post2hb', 'post3hb', 'post1hb', 'post47hb', 'post54hb', 
                                        'post5hb', 'post18hb', 'post20hb', 'post14hb', 'post28hb']
        
        self.LOWEST_FRACTAL_POST_IDS = ['post7lb', 'post16lb', 'post28lb', 'post12lb', 'post23lb', 
    ...
    def process_all_conversations(self, folder_path='json_data'):
        ...
        try:
            ...
            print(f"\nProcessing {len(conversations)} conversations for TST sentiment analysis...")
    ...
    def create_visualizations(self, output_folder='subtree_sentiment_analysis_level4'):
        """Create research-appropriate visualization for fractal dimension TST sentiment analysis"""
        if not self.results:
        ...
        for i, conv_type in enumerate(conversation_types):
            ...
            if level_data:  # Only create plot if there's data
                ...
                # Overlay ALL individual points as scatter plots
        ...
        plot_path = os.path.join(graphs_folder, f'tst_sentiment_research_level{self.max_level}_fractal.png')
        plt.savefig(plot_path, dpi=300, bbox_inches='tight', facecolor='white', 
                    edgecolor='none', format='png')
        plt.show()
        
        print(f"Fractal Dimension TST visualization saved to: {plot_path}")
        return plot_path

# Main execution function
def run_fractal_tst_sentiment_analysis(max_level=4):
    """Main function to run fractal dimension filtered TST sentiment analysis"""
    print(f"=== FRACTAL DIMENSION TST SENTIMENT ANALYSIS (LEVEL CAP: {max_level}) ===")
    ...
    print("- Visualization: Research-appropriate box plots comparing fractal dimensions")
    ...
    print(f"\n=== FRACTAL DIMENSION TST SENTIMENT ANALYSIS COMPLETE ===")
    ...
    print(f"- Research-appropriate visualization comparing fractal dimensions")
    
    return analyzer

# Run the analysis
if __name__ == "__main__":
    analyzer = run_fractal_tst_sentiment_analysis(max_level=4)