...
class FractalDimensionSentimentAnalyzer:
    """
    Analyzes general sentiment across conversation levels 1-4 for specific fractal dimension post IDs
    Uses level cap instead of time window
    Preserves individual values and uses medians/ranges
    FILTERED: Only processes specified highest and lowest fractal dimension post IDs
    ...
    def __init__(self, max_level=4):
        ...
        # Define the specific post IDs to analyze (original posts fractal dimensions)
        self.HIGHEST_FRACTAL_POST_IDS = ['post50hb', 'post5hb', 'post2hb', 'post57hb', 'post18hb', 
                                        'post29hb', 'post1hb', 'post47hb', 'post34hb', 'post33hb']
        
        self.LOWEST_FRACTAL_POST_IDS = ['post7lb', 'post10lb', 'post21lb', 'post22lb', 'post51lb', 
                                       'post20lb', 'post47lb', 'post25lb', 'post54lb', 'post5lb']
        
        # Combine all target post IDs
        self.TARGET_POST_IDS = set(self.HIGHEST_FRACTAL_POST_IDS + self.LOWEST_FRACTAL_POST_IDS)
    ...
    def load_conversations(self, folder_path='json_data'):
        """Load Reddit conversation files - FILTERED to only target post IDs"""
        conversations = []
        ...
        # Get all JSON files with reddit_comments in name
        json_files = [f for f in os.listdir(folder_path) 
                      if f.endswith('.json') and 'reddit_comments' in f]
        
        print(f"Found {len(json_files)} reddit comment files in {folder_path}")
        print(f"Filtering to only process {len(self.TARGET_POST_IDS)} target post IDs...")
        ...
        for json_file in json_files:
            ...
            # Only process if post_id is in our target list
            if post_id in self.TARGET_POST_IDS:
                ...
                try:
                    ...
                    if post_id in self.HIGHEST_FRACTAL_POST_IDS:
                        conversation_type = "highest fractal dimension"
                    elif post_id in self.LOWEST_FRACTAL_POST_IDS:
                        conversation_type = "lowest fractal dimension"
                    else:
                        # This shouldn't happen, but just in case
                        conversation_type = "unknown fractal dimension"
        ...
        print(f"  - Skipped: {skipped_count} files (not in target post IDs)")
        print(f"  - Target highest fractal: {len(self.HIGHEST_FRACTAL_POST_IDS)} post IDs")
        print(f"  - Target lowest fractal: {len(self.LOWEST_FRACTAL_POST_IDS)} post IDs")
    ...
    def process_all_conversations(self, folder_path='json_data'):
        ...
        try:
            ...
            print(f"\nProcessing {len(conversations)} conversations for essential sentiment analysis...")
            print(f"Level cap: {self.max_level}")
            print("Filter: Only specified fractal dimension post IDs")
    ...
    def create_summary_statistics(self):
        ...
        print(f"\n=== FRACTAL DIMENSION SENTIMENT ANALYSIS (LEVEL CAP: {self.max_level}) ===")
        print(f"Total conversations analyzed: {len(self.results)}")
        print(f"FILTERED TO: Specific fractal dimension post IDs only")
        ...
        # By conversation type (fractal dimension)
        print(f"\nBy Fractal Dimension Category:")
    ...
    def create_research_visualizations(self, output_folder='original_posts_sentiment_analysis_level4'):
        """Create research-appropriate visualizations for fractal dimension analysis"""
        if not self.results:
        ...
        fig, axes = plt.subplots(1, 2, figsize=(12, 5))  # Slightly wider for fractal dimension labels
        ...
        plot_path = os.path.join(graphs_folder, f'research_sentiment_analysis_level{self.max_level}_fractal.png')
        plt.savefig(plot_path, dpi=300, bbox_inches='tight', facecolor='white', edgecolor='none')
        
        # Also save as PDF for research papers
        pdf_path = os.path.join(graphs_folder, f'research_sentiment_analysis_level{self.max_level}_fractal.pdf')
        plt.savefig(pdf_path, bbox_inches='tight', facecolor='white', edgecolor='none')
        
        plt.show()
        
        print(f"Fractal Dimension Original Posts visualization saved to:")
    ...
    def create_visualizations(self, output_folder='original_posts_sentiment_analysis_level4'):
        ...
        colors = {
            'highest fractal dimension': plt.cm.viridis(np.linspace(0, 0.8, len(levels))),
            'lowest fractal dimension': plt.cm.plasma(np.linspace(0, 0.8, len(levels)))
        }
        ...
        if not os.path.exists(graphs_folder):
            os.makedirs(graphs_folder)
        plot_path = os.path.join(graphs_folder, f'original_posts_sentiment_by_fractal_dimension_level{self.max_level}.png')
...
def run_fractal_sentiment_analysis(max_level=4):
    """Main function to run fractal dimension filtered sentiment analysis with original visualization"""
    print(f"=== FRACTAL DIMENSION SENTIMENT ANALYSIS (LEVEL CAP: {max_level}) ===")
    ...
    print("- FILTERED TO: Only specific fractal dimension post IDs")
    print("- Highest fractal dimension: 10 post IDs")
    print("- Lowest fractal dimension: 10 post IDs")
    print("- UPDATED: Visualization now separates Highest vs Lowest Fractal Dimension conversation types")
    ...
    analyzer = FractalDimensionSentimentAnalyzer(max_level=max_level)
    
    # Process all conversations (filtered to target post IDs)
    ...
    print(f"\n=== FRACTAL DIMENSION SENTIMENT ANALYSIS COMPLETE ===")
    print(f"Generated files with essential sentiment metrics:")
    ...
    print(f"- UPDATED: Visualization shows Highest vs Lowest Fractal Dimension conversation types separately")
    print(f"- Filtered to: Specific fractal dimension post IDs only")
...
def run_research_fractal_sentiment_analysis(max_level=4):
    """Main function to run fractal dimension filtered sentiment analysis with research-appropriate visualization"""
    print(f"=== RESEARCH FRACTAL DIMENSION SENTIMENT ANALYSIS (LEVEL CAP: {max_level}) ===")
    ...
    print("- Filtered to: Specific fractal dimension post IDs")
    ...
    print(f"\n=== RESEARCH FRACTAL DIMENSION VISUALIZATION COMPLETE ===")
    print(f"Output files:")
    print(f"- research_sentiment_analysis_level4_fractal.png (300 DPI)")
    print(f"- research_sentiment_analysis_level4_fractal.pdf (vector format)")
    print(f"- Ready for research paper inclusion")
    print(f"- Filtered to: Highest vs Lowest fractal dimension posts only")
    
    return analyzer


# Combined execution function
def run_complete_fractal_sentiment_analysis(max_level=4):
    """Run both original and research visualizations for fractal dimension analysis"""
    print(f"=== COMPLETE FRACTAL DIMENSION SENTIMENT ANALYSIS (LEVEL CAP: {max_level}) ===")
    ...
    print(f"\n=== COMPLETE FRACTAL DIMENSION ANALYSIS FINISHED ===")
    ...
    print(f"- original_posts_sentiment_by_fractal_dimension_level4.png (colored)")
    print(f"- research_sentiment_analysis_level4_fractal.png (B&W, 300 DPI)")
    print(f"- research_sentiment_analysis_level4_fractal.pdf (vector)")
    print(f"- Filtered to: Highest vs Lowest fractal dimension posts only")
...
if __name__ == "__main__":
    ...
    # analyzer = run_fractal_sentiment_analysis(max_level=4)
    
    # Option 2: Research-only analysis with B&W visualization
    analyzer = run_research_fractal_sentiment_analysis(max_level=4)
    
    # Option 3: Complete analysis with both visualizations
    # analyzer = run_complete_fractal_sentiment_analysis(max_level=4)