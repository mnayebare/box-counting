...
# Set matplotlib to use research paper-appropriate styling - IMPROVED VISIBILITY
plt.rcParams.update({
    ...
    'legend.fontsize': 18,     # Increased from 14
    'axes.linewidth': 2,       # Reduced from 3 to 2
    'lines.linewidth': 2,      # Reduced from 4 to 2
    'lines.markersize': 6,     # Reduced from 10 to 6
    'xtick.major.width': 2,    # Reduced from 3 to 2
    'ytick.major.width': 2,    # Reduced from 3 to 2
...
class ThreadContourExtractor:
    ...
    def visualize_contour(self, title_suffix="", save_path=None, figsize=(12, 8), dpi=300):
        ...
        # Plot contour line in black with appropriate thickness
        ax.plot(contour[:, 0], contour[:, 1], 'k-', linewidth=2, alpha=0.8, label='Conversation Contour')
        
        # Plot points with appropriate size
        ax.scatter(contour[:, 0], contour[:, 1], c='black', s=8, alpha=0.6, zorder=5)
        
        # Research paper appropriate labels - bold and clear
        ax.set_xlabel('Conversation Depth', fontweight='bold', fontsize=22)
        ax.set_ylabel('Number of comments', fontweight='bold', fontsize=22)
        ax.set_title(f'Conversation Thread Contour {title_suffix}', fontweight='bold', fontsize=24, pad=20)
        
        # Black grid for better readability
        ax.grid(True, alpha=0.3, color='black', linewidth=1)
        
        # Make axis spines appropriate thickness
        for spine in ax.spines.values():
            spine.set_linewidth(2)
            spine.set_color('black')
        
        # Appropriate tick labels
        ax.tick_params(axis='both', which='major', labelsize=16, width=2, length=6, colors='black')
        ...
        try:
            ...
            print(f"SUCCESS: Contour visualization saved as: {save_path}")
            # Verify file was actually created
            if os.path.exists(save_path):
                file_size = os.path.getsize(save_path)
                print(f"File confirmed: {save_path} ({file_size} bytes)")
            else:
                print(f"ERROR: File was not created: {save_path}")
        except Exception as e:
...
def box_counting_fractal_dimension(contour_points, box_sizes=None, save_path=None, 
    ...
    # Plot data points with appropriate thickness
    ax.loglog(1/box_sizes, counts, 'ko-', linewidth=2, markersize=8, 
              markerfacecolor='black', markeredgecolor='black', markeredgewidth=1,
              label='Observed Data')
    
    # Plot fit line with appropriate thickness
    ...
    # Research paper appropriate labels - bold and clear
    ax.set_xlabel('Inverse Box Size (1/ε)', fontweight='bold', fontsize=22)
    ax.set_ylabel('Number of Occupied Boxes N(ε)', fontweight='bold', fontsize=22)
    ax.set_title('Box-Counting Fractal Dimension Analysis', fontweight='bold', fontsize=24, pad=20)
    
    # Black legend with thick border
    legend = ax.legend(loc='upper left', fontsize=18, frameon=True, 
                      facecolor='white', edgecolor='black')
    legend.get_frame().set_facecolor('white')
    legend.get_frame().set_linewidth(2)  # Set border thickness this way
    ...
    try:
        plt.savefig(save_path, dpi=dpi, bbox_inches='tight', 
                   facecolor='white', edgecolor='black')
        print(f"SUCCESS: Fractal analysis plot saved as: {save_path}")
        # Verify file was actually created
        if os.path.exists(save_path):
            file_size = os.path.getsize(save_path)
            print(f"File confirmed: {save_path} ({file_size} bytes)")
        ...
        print(f"ERROR saving fractal image: {e}")
        print(f"Attempted save path: {save_path}")
...
if __name__ == "__main__":
    ...
    contours_dir = "controversial_posts_contour_visualizations"
    fractal_dir = "controversial_posts_fractal_analysis_visualizations"
    subtrees_contours_dir = "subtree_contours_visualizations"
    subtrees_fractal_dir = "subtree_fractal_analysis_visualizations"
    ...
    original_csv_filename = f"original_posts_fractal_analysis.csv"
    if original_posts:
    ...
    subtree_csv_filename = f"subtree_fractal_analysis.csv"
    if subtree_results:
    ...
    print(f"  - Font sizes: 18-24pt with bold styling")
    print(f"  - Line thickness: 4-6pt for visibility")
    print(f"  - Grid lines: Black with transparency")
    print(f"  - Axis spines: 3pt thickness")
    ...
    print(f"\nResults saved to:")
    print(f"  - Controversial posts: {original_csv_filename}")
